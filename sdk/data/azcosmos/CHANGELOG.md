...
* Added Stage 4 of the pure-Go query engine: standalone `TOP` (`SELECT TOP N * FROM c`) now succeeds cross-partition. Per-partition responses are concatenated and truncated at N. Composition with `ORDER BY`, `DISTINCT`, aggregates, or `GROUP BY` is out of scope for this release. See [BYT-9239](https://linear.app/bytebase/issue/BYT-9239).
* Added Stage 5 of the pure-Go query engine: `OFFSET` and `LIMIT` composed with `ORDER BY` — `SELECT … FROM c ORDER BY c.field OFFSET N LIMIT M` now succeeds cross-partition. The gateway rewrites each partition's query to `OFFSET 0 LIMIT (N+M)` so every partition delivers enough rows to cover any interleaving; the engine merges globally, skips the first N, and emits up to M. See [BYT-9239](https://linear.app/bytebase/issue/BYT-9239).
* Added Stage 6 of the pure-Go query engine: `GROUP BY` with any combination of `COUNT`, `SUM`, `MIN`, `MAX`, `AVG` aggregates alongside grouping columns. Per-partition group contributions are merged into a hash map keyed on the canonical JSON of the group-by items; per-alias aggregators reuse the Stage 1 finalizer math. Output is emitted one row per group in the alias order the gateway's plan prescribed. Composition with `ORDER BY`, `TOP`, `OFFSET/LIMIT`, or `DISTINCT` is out of scope for this release. See [BYT-9239](https://linear.app/bytebase/issue/BYT-9239).
